...
class NetStat:
	...
	def netstat_screen(self):
	    ...
	    content=self._load()
            result = [['ID', 'UID', 'LOCAL', 'REMOTE', 'STATE', 'PID', 'PNAME']]
	    for line in content:
	...
	def netstat_data(self):
		...
		nline = {'id':tcp_id, 'uid':uid, 'lhost':l_host, 'lport':l_port, 'rhost':r_host, 'rport':r_port, 'state':state, 'pid':pid, 'pname':exe}
		result.append(nline)
	    return result
